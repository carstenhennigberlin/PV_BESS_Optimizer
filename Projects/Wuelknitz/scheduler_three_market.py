...
def get_daa_schedule(pv_output, daa, p_limit, storage_capacity, p_charge_max, p_discharge_max, number_of_cycles=2, efficiency=0.95, start_soc=0.0, end_soc=0.0):
    """    
    ...
    model.objective = pyo.Objective(expr = sum(daa[t-1] / 4 * (- model.p_charge_daa[t]
                                                            + model.p_discharge_daa[t] 
                                                            - model.p_curtailed_daa[t] 
    ...
    # injection_power = [p_discharge_daa[t] + pv_output[t-1] - p_curtailed[t] - p_charge_daa[t] for t in range(0, len(daa))]
    injection_power = [p_discharge_daa[t] + pv_output[t] - p_curtailed_daa[t] - p_charge_daa[t] for t in range(0, len(daa))]

    return pv_output, daa, p_charge_daa, p_discharge_daa, soc, p_curtailed_daa, injection_power


def get_ida_schedule(ida, pv_output, p_limit, storage_capacity, p_charge_max, p_discharge_max, p_charge_daa, p_discharge_daa, p_curtailed_daa, number_of_cycles=2, efficiency=0.95, start_soc=0.0, end_soc=0.0):
    """
    ...
    #### Testing constraints #####
    # def discharge_at_negative_price_rule(model, t):
    #     if daa[t-1] < 0:
    #         return model.p_discharge_ida[t] == 0.0
    #     else:
    #         return pyo.Constraint.Skip
    # model.discharge_at_negative_price_constraint = pyo.Constraint(model.T, rule=discharge_at_negative_price_rule)
    ...
    model.objective = pyo.Objective(expr = sum(ida[t-1] / 4 * (model.p_discharge_ida[t]
                                                            ...
                                                            + model.p_close_curtailed_daa[t]
                                                            - model.p_curtailed_ida[t]) for t in model.T), sense=pyo.maximize)                              
    ...
    # Calculate the injection power
    injection_power_ida = [p_discharge_ida[t] - p_charge_ida[t]
                       + p_discharge_daa[t] - p_charge_daa[t]
                       + p_close_charge_daa[t] - p_close_discharge_daa[t]
                       + pv_output[t]
                       - p_curtailed_daa[t] + p_close_curtailed_daa[t]
                       - p_curtailed_ida[t] for t in range(0, len(ida))]
    ...
    return ida, p_charge_ida, p_discharge_ida, p_close_charge_daa, p_close_discharge_daa, p_curtailed_ida, \
        p_close_curtailed_daa, p_curtailed_daa_ida, p_charge_daa_ida, p_discharge_daa_ida, soc_ida, injection_power_ida

def get_idc_schedule(idc, pv_output, p_limit, storage_capacity, p_charge_max, p_discharge_max, p_charge_daa_ida, p_discharge_daa_ida, p_curtailed_daa_ida, number_of_cycles=2, efficiency=0.95, start_soc=0.0, end_soc=0.0):
    """
    ...
    # def discharge_at_negative_price_rule(model, t):
    #     if daa[t-1] < 0:
    #         return model.p_discharge_idc[t] == 0.0
    #     else:
    #         return pyo.Constraint.Skip
    # model.discharge_at_negative_price_constraint = pyo.Constraint(model.T, rule=discharge_at_negative_price_rule)
    ...
    model.objective = pyo.Objective(expr = sum(idc[t-1] / 4 * (model.p_discharge_idc[t]
                                                            ...
                                                            + model.p_close_curtailed_daa_ida[t]
                                                            - model.p_curtailed_idc[t]) for t in model.T), sense=pyo.maximize)
    ##### Solve the model #####
    solver = pyo.SolverFactory(SOLVER)
    ...
    # Calculate the injection power
    injection_power_idc = [p_discharge_idc[t] - p_charge_idc[t]
                       + p_discharge_daa_ida[t] - p_charge_daa_ida[t]
                       + p_close_charge_daa_ida[t] - p_close_discharge_daa_ida[t]
                       + pv_output[t]
                       - p_curtailed_daa_ida[t] + p_close_curtailed_daa_ida[t]
                       - p_curtailed_idc[t] for t in range(0, len(idc))]
    ...
    return idc, p_charge_idc, p_discharge_idc, p_close_charge_daa_ida, p_close_discharge_daa_ida, p_curtailed_idc, \
        p_close_curtailed_daa_ida, p_curtailed_daa_ida_idc, p_charge_daa_ida_idc, p_discharge_daa_ida_idc, soc_idc, injection_power_idc
